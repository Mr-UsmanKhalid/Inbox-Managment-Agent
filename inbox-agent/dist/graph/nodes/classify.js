...
export async function classifyNode(state) {
    ...
    if (isMockMode()) {
        ...
        classification = await model.invoke([
            {
                role: "system",
                content: "You classify inbound business emails. Be precise about urgency: refunds, threats to " +
...
function mockClassify(subject, body) {
    const text = `${subject} ${body}`.toLowerCase();
    let category = "general";
    if (/order|shipp|track|deliver/.test(text))
        category = "order_status";
    if (/refund|charge|invoice|bill/.test(text))
        category = "billing";
    if (/terrible|complain|angry|disput|broken|third time/.test(text))
        category = "complaint";
    if (/pricing|plan|enterprise|trial|seats|contract/.test(text))
        category = "sales_inquiry";
    let urgency = 20;
    if (/refund|dispute|bank|third time|immediately|broken/.test(text))
        urgency = 90;
    else if (/order|track|when will/.test(text))
        urgency = 45;
    else if (/trial|pricing|how long/.test(text))
        urgency = 15;
    let sentiment = "neutral";
    if (/terrible|angry|disput|demand/.test(text))
        sentiment = "angry";
    else if (/thanks|please|quick question/.test(text))
        sentiment = "positive";
    return {
        category,
        intent: subject,
