...
async function warmUp() {
    ...
    console.log(`Warmed up ${threads.size} thread(s) in ${isMockMode()
        ? "MOCK LLM"
        : `LIVE (${getActiveProvider()})`} mode.`);
}
const app = express();
app.use(cors({
...
app.get("/api/threads", (_req, res) => {
    const list = Array.from(threads.values()).sort((a, b) => new Date(b.receivedAt).getTime() -
