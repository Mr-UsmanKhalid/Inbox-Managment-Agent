import { z } from "zod";
import { AgentState, ExtractedEntities } from "../../types.js";
...
function mockExtract(from: string, body: string): ExtractedEntities {
  const orderMatch = body.match(/order\s*#?(\d{4,})/i);
  const nameGuess = from.split("@")[0].replace(/[._]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  const requirements: string[] = [];
  if (/refund/i.test(body)) requirements.push("wants a refund");
  if (/sso/i.test(body)) requirements.push("needs SSO");
  if (/trial/i.test(body)) requirements.push("asking about free trial terms");
