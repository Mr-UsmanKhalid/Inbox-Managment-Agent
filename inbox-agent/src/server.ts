...
// database when you need persistence across restarts.
const threads = new Map<string, DashboardThread>();
let connector: Awaited<ReturnType<typeof getEmailConnector>>;
...
async function warmUp() {
  connector = await getEmailConnector();
  ...
  console.log(
    `Warmed up ${threads.size} thread(s) in ${isMockMode() ? "MOCK LLM" : `LIVE (${getActiveProvider()})`} mode. ` +
      `Email connector: ${(process.env.EMAIL_CONNECTOR || "mock").toLowerCase()}.`
  );
}
...
// Populate the in-memory thread table once per process/cold start, before
// the app starts handling requests. Safe to await at module scope in ESM
// ("type": "module" in package.json).
//
// Vercel's zero-config Express support auto-detects this exact file
// (src/server.ts is one of its recognized locations) and deploys the
// default-exported app directly as a single Vercel Function - no
// vercel.json or api/ folder needed. See:
// https://vercel.com/docs/frameworks/backend/express
//
// Locally (or anywhere else without Vercel's runtime), VERCEL isn't set,
// so we fall back to a normal app.listen() on PORT instead.
